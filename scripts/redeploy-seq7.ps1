...
$collConfig = $ccDir + '\collections_config.json'
...
Write-Host 'Packaging sih-contract v2.9 (PDC + Events + Security Hardening)...' -ForegroundColor Cyan
...
$pkgFile = Join-Path $netDir 'sih-contract_2.9.tar.gz'
Remove-Item -Force -ErrorAction SilentlyContinue $pkgFile
$r = & $peer lifecycle chaincode package $pkgFile --path $ccDir --lang node --label sih-contract_2.9 2>&1
Write-Host $r
...
$pkgId = $null
foreach ($line in $installed) { if ($line -match '(sih-contract_2.9:[a-f0-9]+)') { $pkgId = $Matches[1]; break } }
Write-Host 'New Package ID:' $pkgId -ForegroundColor Green

# Approve Seq7 for BEL
Write-Host 'Approving Seq7 for BEL with collections_config...' -ForegroundColor Yellow
$r = & $peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer1.sih26125.local --channelID sihchannel --name sih-contract --version $version --sequence $sequence --tls --cafile $ordCa --package-id $pkgId --signature-policy $policy --collections-config $collConfig 2>&1
Write-Host $r

# Approve Seq7 for Auditor
Write-Host 'Approving Seq7 for Auditor with collections_config...' -ForegroundColor Yellow
$env:CORE_PEER_LOCALMSPID = 'AuditorMSP'
$env:CORE_PEER_MSPCONFIGPATH = $audAdminMsp
$env:CORE_PEER_ADDRESS = 'localhost:8051'
$env:CORE_PEER_TLS_ROOTCERT_FILE = $audTls
...
# Approve Seq7 for Contractor
Write-Host 'Approving Seq7 for Contractor with collections_config...' -ForegroundColor Yellow
$env:CORE_PEER_LOCALMSPID = 'ContractorMSP'
$env:CORE_PEER_MSPCONFIGPATH = $conAdminMsp
$env:CORE_PEER_ADDRESS = 'localhost:9051'
$env:CORE_PEER_TLS_ROOTCERT_FILE = $conTls
...
# Commit Seq7
Write-Host 'Committing Seq7 with collections_config...' -ForegroundColor Yellow
$env:CORE_PEER_LOCALMSPID = 'BELMSP'
$env:CORE_PEER_MSPCONFIGPATH = $belAdminMsp
$env:CORE_PEER_ADDRESS = 'localhost:7051'
$env:CORE_PEER_TLS_ROOTCERT_FILE = $belTls
$r = & $peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer1.sih26125.local --channelID sihchannel --name sih-contract --version $version --sequence $sequence --tls --cafile $ordCa --peerAddresses localhost:7051 --tlsRootCertFiles $belTls --peerAddresses localhost:8051 --tlsRootCertFiles $audTls --peerAddresses localhost:9051 --tlsRootCertFiles $conTls --signature-policy $policy --collections-config $collConfig 2>&1
Write-Host $r
