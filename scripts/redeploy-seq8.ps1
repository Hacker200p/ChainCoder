...
Write-Host 'Packaging sih-contract v2.10...' -ForegroundColor Cyan
...
$pkgFile = Join-Path $netDir 'sih-contract_2.10.tar.gz'
Remove-Item -Force -ErrorAction SilentlyContinue $pkgFile
$r = & $peer lifecycle chaincode package $pkgFile --path $ccDir --lang node --label sih-contract_2.10 2>&1
Write-Host $r
...
$pkgId = $null
foreach ($line in $installed) { if ($line -match '(sih-contract_2.10:[a-f0-9]+)') { $pkgId = $Matches[1]; break } }
Write-Host 'New Package ID:' $pkgId -ForegroundColor Green

# Approve Seq8 for BEL
Write-Host 'Approving Seq8 for BEL...' -ForegroundColor Yellow
...
# Approve Seq8 for Auditor
Write-Host 'Approving Seq8 for Auditor...' -ForegroundColor Yellow
$env:CORE_PEER_LOCALMSPID = 'AuditorMSP'
$env:CORE_PEER_MSPCONFIGPATH = $audAdminMsp
$env:CORE_PEER_ADDRESS = 'localhost:8051'
$env:CORE_PEER_TLS_ROOTCERT_FILE = $audTls
...
# Approve Seq8 for Contractor
Write-Host 'Approving Seq8 for Contractor...' -ForegroundColor Yellow
$env:CORE_PEER_LOCALMSPID = 'ContractorMSP'
$env:CORE_PEER_MSPCONFIGPATH = $conAdminMsp
$env:CORE_PEER_ADDRESS = 'localhost:9051'
$env:CORE_PEER_TLS_ROOTCERT_FILE = $conTls
...
# Commit Seq8
Write-Host 'Committing Seq8...' -ForegroundColor Yellow
$env:CORE_PEER_LOCALMSPID = 'BELMSP'
$env:CORE_PEER_MSPCONFIGPATH = $belAdminMsp
$env:CORE_PEER_ADDRESS = 'localhost:7051'
$env:CORE_PEER_TLS_ROOTCERT_FILE = $belTls
