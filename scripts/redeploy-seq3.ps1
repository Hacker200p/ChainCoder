...
Write-Host 'Packaging sih-contract v2.5...' -ForegroundColor Cyan
...
$r = & $peer lifecycle chaincode package (Join-Path $netDir 'sih-contract_2.5.tar.gz') --path $ccDir --lang node --label sih-contract_2.5 2>&1
Write-Host $r

Write-Host 'Installing on BEL peer...' -ForegroundColor Yellow
$r = & $peer lifecycle chaincode install (Join-Path $netDir 'sih-contract_2.5.tar.gz') 2>&1
Write-Host $r
...
$pkgId = $null
foreach ($line in $installed) { if ($line -match '(sih-contract_2.5:[a-f0-9]+)') { $pkgId = $Matches[1]; break } }
Write-Host 'New Package ID:' $pkgId -ForegroundColor Green

# Approve Seq3 for BEL
Write-Host 'Approving Seq3 for BEL...' -ForegroundColor Yellow
...
# Approve Seq3 for Auditor
Write-Host 'Approving Seq3 for Auditor...' -ForegroundColor Yellow
$env:CORE_PEER_LOCALMSPID = 'AuditorMSP'
$env:CORE_PEER_MSPCONFIGPATH = $audAdminMsp
$env:CORE_PEER_ADDRESS = 'localhost:8051'
$env:CORE_PEER_TLS_ROOTCERT_FILE = $audTls
...
# Approve Seq3 for Contractor
Write-Host 'Approving Seq3 for Contractor...' -ForegroundColor Yellow
$env:CORE_PEER_LOCALMSPID = 'ContractorMSP'
$env:CORE_PEER_MSPCONFIGPATH = $conAdminMsp
$env:CORE_PEER_ADDRESS = 'localhost:9051'
$env:CORE_PEER_TLS_ROOTCERT_FILE = $conTls
...
# Commit Seq3
Write-Host 'Committing Seq3...' -ForegroundColor Yellow
$env:CORE_PEER_LOCALMSPID = 'BELMSP'
$env:CORE_PEER_MSPCONFIGPATH = $belAdminMsp
$env:CORE_PEER_ADDRESS = 'localhost:7051'
$env:CORE_PEER_TLS_ROOTCERT_FILE = $belTls
